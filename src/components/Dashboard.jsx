...
export default function Dashboard({
  ...
  return (
    <div className="space-y-8">
      ...
      <div className={cn(
        'rounded-[2rem] p-6 backdrop-blur-[10px] border shadow-ambient flex items-center gap-6 animate-in fade-in slide-in-from-top-4 duration-700',
        weatherState.theme
      )}>
        <div className="shrink-0 drop-shadow-md">{weatherState.icon}</div>
        <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        {/* Monthly P&L */}
        <div className={cn(
          'glass-effect-dark p-6 rounded-3xl shadow-ambient transition-colors flex flex-col justify-center',
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-on-surface-variant font-medium text-xs mb-1 uppercase tracking-wide flex items-center gap-1">
                Monthly P&L {isDeficit && <AlertTriangle className="w-3.5 h-3.5 text-rose-500" />}
              </p>
              <span className={cn('text-2xl font-manrope font-bold', isDeficit ? 'text-rose-600' : 'text-emerald-600')}>
                {isDeficit ? '-' : '+'}₹{Math.abs(monthlyPL).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              </span>
              <p className={cn('text-xs font-semibold mt-0.5', isDeficit ? 'text-rose-500' : 'text-emerald-500')}>
        ...
        {/* Total Expenses */}
        <div className="bg-gradient-to-br from-primary to-primary-container p-6 rounded-3xl shadow-ambient text-white relative overflow-hidden group">
          ...
          <p className="text-primary-fixed-dim text-xs font-semibold uppercase tracking-wider mb-2">Total Expenses</p>
          <h3 className="text-2xl lg:text-3xl font-manrope font-bold">
        ...
        <div className="bg-gradient-to-br from-sky-500 to-cyan-600 p-6 rounded-3xl shadow-ambient text-white relative overflow-hidden group col-span-2 lg:col-span-1">
          ...
          <h3 className="text-2xl lg:text-3xl font-manrope font-bold">{txCount}</h3>
          <p className="text-sky-200 text-xs font-medium mt-1">total recorded</p>
        </div>
      </div>
